...
FROM 
   ...
   FROM viagens 
   GROUP BY ida 
   ORDER BY COUNT(ida) DESC) 
WHERE ROWNUM = 1;

-- pega as idas e ordena pela contagem delas, 
-- ai para pegar a que mais aparece basta ordenar descendente e pegar a primeira linha

-- Cidade que mais aparece na volta <<<<<<<<<<<<

SELECT * 
FROM 
   (SELECT volta, count(volta) 
   FROM viagens 
   GROUP BY volta 
   ORDER BY COUNT(volta) DESC) 
WHERE ROWNUM = 1;

-- pega as voltas e ordena pela contagem delas, 
-- ai para pegar a que mais aparece basta ordenar descendente e pegar a primeira linha
